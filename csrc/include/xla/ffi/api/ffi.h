...
template <typename T> class ErrorOr : public Expected<T, Error> {
public:
...
template <DataType dtype> struct always_false : std::false_type {};

template <DataType dtype> struct DataTypeToNative {
...
#define XLA_FFI_REGISTER_DATATYPE_MAPPING(data_type_value, actual_type)        \
  template <> struct DataTypeToNative<data_type_value> {                       \
...
template <DataType dtype> constexpr bool IsComplexType() {
...
template <DataType dtype, size_t rank = internal::kDynamicRank> class Buffer {
public:
...
XLA_FFI_ATTRIBUTE_ALWAYS_INLINE std::optional<Buffer<dtype, rank>>
DecodeBuffer(XLA_FFI_Buffer *buf, DiagnosticEngine &diagnostic) {
...
template <> struct ArgBinding<AnyBuffer> {
  using Arg = AnyBuffer;
};

template <DataType dtype, size_t rank> struct ArgBinding<Buffer<dtype, rank>> {
...
template <> struct RetBinding<Result<AnyBuffer>> {
  using Ret = AnyBuffer;
};
...
template <> struct ArgDecoding<AnyBuffer> {
...
template <DataType dtype, size_t rank> struct ArgDecoding<Buffer<dtype, rank>> {
  XLA_FFI_ATTRIBUTE_ALWAYS_INLINE
  static std::optional<Buffer<dtype, rank>>
  Decode(XLA_FFI_ArgType type, void *arg, DiagnosticEngine &diagnostic) {
    if (XLA_FFI_PREDICT_FALSE(type != XLA_FFI_ArgType_BUFFER)) {
      return diagnostic.Emit("Wrong argument type: expected ")
             << XLA_FFI_ArgType_BUFFER << " but got " << type;
    }
...
template <> struct RetDecoding<AnyBuffer> {
  XLA_FFI_ATTRIBUTE_ALWAYS_INLINE
  static std::optional<Result<AnyBuffer>>
  Decode(XLA_FFI_RetType type, void *ret, DiagnosticEngine &diagnostic) {
...
template <DataType dtype, size_t rank> struct RetDecoding<Buffer<dtype, rank>> {
  XLA_FFI_ATTRIBUTE_ALWAYS_INLINE
  static std::optional<Result<Buffer<dtype, rank>>>
  Decode(XLA_FFI_RetType type, void *ret, DiagnosticEngine &diagnostic) {
    if (XLA_FFI_PREDICT_FALSE(type != XLA_FFI_RetType_BUFFER)) {
      return diagnostic.Emit("Wrong result type: expected ")
             << XLA_FFI_RetType_BUFFER << " but got " << type;
...
#define XLA_FFI_REGISTER_ARRAY_ATTR_DECODING(T, TYPE)                          \
  template <> struct AttrDecoding<Span<const T>> {                             \
...
template <typename T> struct AttrDecoding<Pointer<T>> {
  using Type = T *;
...
template <ExecutionStage stage> struct ResultEncoding<stage, Error> {
...
template <typename T> struct CtxDecoding<PlatformStream<T>> {
  using Type = T;
...
template <> struct CtxDecoding<ScratchAllocator> {
...
//                     .To([](MyData* data) { ... });
template <typename T> struct CtxDecoding<UserData<T>> {
  using Type = T *;
...
//                     .To([](MyState* state) { ... });
template <typename T> struct CtxDecoding<State<T>> {
  using Type = T *;
