...
public:
  virtual void Start(const std::string &iReport) = 0;
  ...
  virtual void SetColumnNames(const ColumnNames &iCol) = 0;
...
public:
  ...
  void SetColumnNames(const ColumnNames &iCol) override { m_ColNames = iCol; }
  /**
...
public:
  void Start(const std::string &iReport) override {
...
public:
  ...
  void SetColumnNames(const ColumnNames &iCol) {
    if (m_EnablePerfoStep)
      m_Perfostep->SetColumnNames(iCol);
  }

  void Start(const std::string &iReport, const ColumnNames &iCol = {}) {
    if (m_EnablePerfoStep)
      m_Perfostep->Start(iReport);
  }
  double Stop() {
    if (m_EnablePerfoStep)
