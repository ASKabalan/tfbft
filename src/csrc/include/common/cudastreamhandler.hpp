...
public:
  ...
  void join(cudaStream_t finalStream) {
    std::for_each(m_streams.begin(), m_streams.end(),
                  [this, finalStream](cudaStream_t stream) {
                    cudaEvent_t event;
  ...
  void Fork(cudaStream_t dependentStream, int N) {
    ...
    std::for_each(m_streams.end() - N, m_streams.end(),
                  [this, dependentStream](cudaStream_t stream) {
                    cudaEvent_t event;
                    cudaEventCreate(&event);
  ...
  auto getIterator() {
    return StreamIterator(m_streams.begin(), m_streams.end());
  }
  ...
  public:
    StreamIterator(std::vector<cudaStream_t>::iterator begin,
...
public:
  ...
  void AddStreams(int numStreams) {
    CudaStreamHandlerImpl::instance().AddStreams(numStreams);
  }

  void join(cudaStream_t finalStream) {
    CudaStreamHandlerImpl::instance().join(finalStream);
  }

  void Fork(cudaStream_t cudastream, int N) {
    CudaStreamHandlerImpl::instance().Fork(cudastream, N);
  }
