...
public:
    // Constructor
    CommIterator(std::vector<StageCommPair> comms) : m_comms(comms), m_it(m_comms.begin()) {}
...
public:
    ...
    ~ButterflyCommIndexImpl() {
        for (auto &comms : comms_db) {
            for (auto &comm : comms.second) {
                ncclCommDestroy(comm.second);
            }
        }
    }
